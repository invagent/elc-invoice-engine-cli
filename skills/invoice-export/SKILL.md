...
| 参数 | 提取方式 | 默认值 |
|------|----------|--------|
| `company` | 直接识别如 "BU-00008" | 读 ~/.elc/.env 的 X_COMPANY_ID，已配置则无需传 |
...
template 已有默认值，通常不需要传入。日期必须转化为具体的 `--from/--to`。
...
**Claude 执行：**
1. 解析：最近2周 → `--from 2026-06-17 --to 2026-07-01`，company=BU-00008
2. 检查映射表 → 已初始化 → 直接执行导出
3. 运行：`elc export invoice --from 2026-06-17 --to 2026-07-01 --company BU-00008`
4. 报告：✓ 已导出 N 张发票到 ~/Downloads/invoice_export_20260617_20260701.xlsx

---

**用户输入：**
> 导出上个月组织为 BU-00008 的发票数据

**Claude 执行：**
1. 解析：上个月 → `--from 2026-06-01 --to 2026-06-30`，company=BU-00008
2. 检查映射表 → 未初始化 → 运行 `elc export init` 采集样本
3. 读取样本，推断映射 JSON
4. 运行 `elc export save-mapping` 保存
5. 运行：`elc export invoice --from 2026-06-01 --to 2026-06-30 --company BU-00008`
6. 报告：✓ 已导出 N 张发票到 ~/Downloads/invoice_export_20260601_20260630.xlsx
